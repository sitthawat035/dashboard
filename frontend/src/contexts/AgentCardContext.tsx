...
import { statusApi, gatewayApi } from '../utils/api';
...
interface AgentCardContextValue {
  ...
  onControl: (gw: string, action: string) => Promise<{ ok: boolean; status: number; data?: any }>;
...
export function AgentCardProvider({
  ...
  const handleControlClick = useCallback(async (gwKey: string, action: string) => {
    if (action === 'start') {
      setIsStarting(true);
      ...
      // Call control and check for immediate failure (HTTP 500 = process exited immediately)
      const result = await onControl(gwKey, action);
      if (result && !result.ok) {
        // Backend detected immediate exit — fail fast, no need to poll
        setIsStarting(false);
        setStartFailed(true);
        clearStartingPoll();
        return;
      ...
      // Fallback safety timer: 60s (was 180s) before showing failure
  ...
  const sendChat = useCallback(async () => {
    ...
    // Emit via Socket.IO for streaming response
    socketManager.emit('chat_message_send', {
      agent_key: gw,
      message: msg,
    });
