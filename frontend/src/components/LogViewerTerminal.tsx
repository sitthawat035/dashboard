...
const LogViewerTerminal: React.FC<LogViewerTerminalProps> = ({ agentId }) => {
  ...
  useEffect(() => {
    ...
    // Initial fit with retry — xterm needs a few attempts to settle
    let fitAttempts = 0;
    const fitInterval = setInterval(() => {
      try {
        fitAddon.fit();
        fitAttempts++;
        if (fitAttempts >= 5) clearInterval(fitInterval);
      } catch { /* ignore */ }
    }, 100);
    setTimeout(() => clearInterval(fitInterval), 600);
    ...
    const handleLogError = (data: { agent_id: string; error: string; message?: string }) => {
      if (data.agent_id !== agentId || !termRef.current) return;
      const errorMsg = data.message || data.error || 'Unknown error';
      termRef.current.writeln(`\x1b[1;31m[ERROR] ${errorMsg}\x1b[0m`);
      if (!isScrollLockedRef.current) termRef.current.scrollToBottom();
    };
    ...
    socketManager.on('gateway_log_error', handleLogError);
    ...
    return () => {
      ...
      socketManager.off('gateway_log_error', handleLogError);
