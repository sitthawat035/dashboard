...
const ConversationLog: React.FC<ConversationLogProps> = ({ agentId, agentName }) => {
  ...
  useSocketEvents({
    onSubagentConversation: useCallback((data: ConvMessage) => {
      if (autoRefresh) {
        setMessages(prev => [...prev, { ts: data.ts, text: data.text }]);
        setIsStreaming(true);
      }
    }, [autoRefresh]),
  });
  ...
  }, [autoRefresh, agentId, isStreaming]);
  ...
  return (
    <div className="conv-log-wrapper">
      <div className="conv-toolbar">
        ...
        <span className={isStreaming ? 'conv-streaming-indicator active' : 'conv-streaming-indicator'}>
