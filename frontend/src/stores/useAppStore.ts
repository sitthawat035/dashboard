...
interface AppState {
  ...
  control: (gw: string, action: string) => Promise<{ ok: boolean; status: number; data?: any }>;
...
export const useAppStore = create<AppState>((set, get) => ({
  ...
  control: async (gw, action) => {
    const result = await gatewayApi.control(gw, action);
    if (result.status === 401) { set({ isLoggedIn: false }); return result; }
    // Only schedule loadStatus for non-immediate-exit starts (success or stop/restart)
