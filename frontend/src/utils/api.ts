...
triggerCronJob: (jobId: string): Promise<any> =>
  ...
  // Sessions
  listSessions: (page = 1): Promise<any> =>
    api.get('/api/hermes/sessions', { params: { page } }).then((r) => r.data),

  deleteSession: (sessionId: string): Promise<any> =>
    api.post(`/api/hermes/sessions/${sessionId}/delete`).then((r) => r.data),

  searchSessions: (q: string): Promise<any> =>
    api.get('/api/hermes/sessions/search', { params: { q } }).then((r) => r.data),

  // Analytics
  getAnalytics: (period = '30d'): Promise<any> =>
    api.get('/api/hermes/analytics', { params: { period } }).then((r) => r.data),

  // Models
  listModels: (): Promise<any> =>
    api.get('/api/hermes/models').then((r) => r.data),

  switchModel: (model: string): Promise<any> =>
    api.post('/api/hermes/models/switch', { model }).then((r) => r.data),

  // Logs
  getLogs: (params: { type?: string; level?: string; component?: string; lines?: number } = {}): Promise<any> =>
    api.get('/api/hermes/logs', { params }).then((r) => r.data),

  // Skills
  listSkills: (): Promise<any> =>
    api.get('/api/hermes/skills').then((r) => r.data),

  toggleSkill: (skill: string, enabled: boolean): Promise<any> =>
    api.post('/api/hermes/skills/toggle', { skill, enabled }).then((r) => r.data),

  // Plugins
  listPlugins: (): Promise<any> =>
    api.get('/api/hermes/plugins').then((r) => r.data),

  togglePlugin: (plugin: string, enabled: boolean): Promise<any> =>
    api.post('/api/hermes/plugins/toggle', { plugin, enabled }).then((r) => r.data),

  installPlugin: (gitUrl: string, force?: boolean, enableAfterInstall?: boolean): Promise<any> =>
    api.post('/api/hermes/plugins/install', { git_url: gitUrl, force, enable_after_install: enableAfterInstall }).then((r) => r.data),

  // Config
  getConfig: (): Promise<any> =>
    api.get('/api/hermes/config').then((r) => r.data),

  saveConfig: (config: any): Promise<any> =>
    api.post('/api/hermes/config/save', config).then((r) => r.data),

  // Gateway
  restartGateway: (): Promise<any> =>
    api.post('/api/hermes/gateway/restart').then((r) => r.data),

  updateHermes: (): Promise<any> =>
    api.post('/api/hermes/gateway/update').then((r) => r.data),

  // Cron create
  createCronJob: (name: string, prompt: string, schedule: string): Promise<any> =>
    api.post('/api/hermes/cron/create', { name, prompt, schedule }).then((r) => r.data),

  // Profiles
  listProfiles: (): Promise<any> =>
    api.get('/api/hermes/profiles').then((r) => r.data),

  // Keys
  getKeys: (): Promise<any> =>
    api.get('/api/hermes/keys').then((r) => r.data),
};
