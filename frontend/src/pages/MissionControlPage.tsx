...
// ── Layout constants ────────────────────────────────────────────
const MC_CENTER_HUB_ID = import.meta.env.VITE_MC_CENTER_HUB_ID || 'pudding';

// Fixed node positions for the agent map (radial + center hub)
const NODE_POSITIONS: Record<string, { x: number; y: number }> = {
  [MC_CENTER_HUB_ID]: { x: 200, y: 180 }, // center hub
};
...
function getPositions(ids: string[]): Record<string, { x: number; y: number }> {
  ...
  const rimIds = ids.filter((id) => id !== MC_CENTER_HUB_ID);
