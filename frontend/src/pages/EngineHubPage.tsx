...
export default function EngineHubPage() {
  ...
  const runEngine = async (id: string) => {
    try {
      ...
      if (id === 'full-pipeline') {
        if (pipelineOptions.scheduleMode === 'scheduled') {
          options = {
            mode: 'scheduled',
            schedule_time: pipelineOptions.scheduleTime,
            repeat: pipelineOptions.repeat,
          };
        } else {
          options = {
            mode: 'now',
            niche: pipelineOptions.niche === 'custom' ? pipelineOptions.customNiche : pipelineOptions.niche,
            tone: pipelineOptions.tone,
            content_type: pipelineOptions.contentType,
            word_count: pipelineOptions.wordCount,
          };
        }
      }
    ...
    <div className={`eh-tabs-container ${fullscreen ? 'eh-fullscreen' : ''}`}>
      ...
      <div className="eh-main">
        ...
        <div className="eh-main-controls">
          {(activeEngine === 'lookforward' || activeEngine === 'veo-gen' || activeEngine === 'shopee-scraper' || activeEngine === 'fb-poster' || activeEngine === 'image-gen' || activeEngine === 'trend-scan' || activeEngine === 'full-pipeline') && (
            <button
        ...
        {/* Full Pipeline Options Panel */}
        {showOptions && activeEngine === 'full-pipeline' && (
          <div className="eh-options-panel">
            <div className="eh-options-header">
              <Zap size={16} />
              <span>Full Automated Pipeline — Options</span>
            </div>
            <div className="eh-options-body">

              {/* Run Mode */}
              <div className="eh-option-group">
                <label>🚀 Run Mode / โหมดการรัน</label>
                <div style={{ display: 'flex', gap: '12px', marginTop: '4px' }}>
                  {(['now', 'scheduled'] as const).map(m => (
                    <label key={m} style={{ display: 'flex', alignItems: 'center', gap: '6px', cursor: 'pointer', padding: '6px 14px', borderRadius: '8px', border: `1px solid ${pipelineOptions.scheduleMode === m ? 'var(--primary)' : 'rgba(255,255,255,0.1)'}`, background: pipelineOptions.scheduleMode === m ? 'rgba(0,242,255,0.08)' : 'transparent', transition: 'all 0.2s' }}>
                      <input type="radio" name="pipe-mode" value={m} checked={pipelineOptions.scheduleMode === m} onChange={() => setPipelineOptions(p => ({ ...p, scheduleMode: m }))} style={{ display: 'none' }} />
                      {m === 'now' ? <Zap size={14} /> : <Calendar size={14} />}
                      {m === 'now' ? 'Run Now (ทันที)' : 'Scheduled (ตั้งเวลา)'}
                    </label>
                  ))}
                </div>
              </div>
              ...
              {pipelineOptions.scheduleMode === 'scheduled' && (
                <div className="eh-option-row">
                  <div className="eh-option-group">
                    <label>⏰ Schedule Time / เวลาเริ่ม Pipeline</label>
                    <input
                      type="time"
                      value={pipelineOptions.scheduleTime}
                      onChange={(e) => setPipelineOptions(p => ({ ...p, scheduleTime: e.target.value }))}
                    />
                  </div>
                  <div className="eh-option-group">
                    <label>🔁 Repeat / ทำซ้ำ</label>
                    <select
                      value={pipelineOptions.repeat}
                      onChange={(e) => setPipelineOptions(p => ({ ...p, repeat: e.target.value as any }))}
                    >
                      <option value="once">Once / ครั้งเดียว</option>
                      <option value="daily">Daily / ทุกวัน</option>
                      <option value="weekdays">Weekdays / จันทร์-ศุกร์</option>
              ...
              {/* Lookforward Settings (shown in both modes) */}
              <div style={{ padding: '10px 14px', borderRadius: '8px', background: 'rgba(167,139,250,0.06)', border: '1px solid rgba(167,139,250,0.15)', marginBottom: '12px' }}>
                <div style={{ fontSize: '0.75rem', textTransform: 'uppercase', color: '#a78bfa', marginBottom: '10px', fontWeight: 700 }}>🧠 Lookforward Content Settings</div>

                <div className="eh-option-group" style={{ marginBottom: '10px' }}>
                  <label>📡 Trend Scan Niche / หมวดหมู่ Trend</label>
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginTop: '6px' }}>
                    {TREND_NICHES.map(n => (
                      <button
                        key={n.key}
                        onClick={() => setPipelineOptions(p => ({ ...p, niche: n.key }))}
                        style={{
                          padding: '5px 12px',
                          borderRadius: '16px',
                          border: `1px solid ${pipelineOptions.niche === n.key ? 'var(--primary)' : 'rgba(255,255,255,0.1)'}`,
                          background: pipelineOptions.niche === n.key ? 'rgba(0,242,255,0.12)' : 'rgba(255,255,255,0.02)',
                          color: pipelineOptions.niche === n.key ? 'var(--primary)' : 'var(--text-secondary)',
                          fontSize: '0.72rem',
                          fontWeight: 600,
                          cursor: 'pointer',
                        }}
                  ...
                  {pipelineOptions.niche === 'custom' && (
                    <input
                      type="text"
                      placeholder="Custom niche description..."
                      value={pipelineOptions.customNiche}
                      onChange={(e) => setPipelineOptions(p => ({ ...p, customNiche: e.target.value }))}
                      style={{ marginTop: '8px' }}
                    />
                  )}
                </div>
                ...
                <div className="eh-option-row">
                  <div className="eh-option-group">
                    <label>Tone / โทน</label>
                    <select
                      value={pipelineOptions.tone}
                      onChange={(e) => setPipelineOptions(p => ({ ...p, tone: e.target.value as any }))}
                    >
                      <option value="professional">Professional</option>
                      <option value="casual">Casual / สบายๆ</option>
                      <option value="educational">Educational / สอนหน่อย</option>
                  ...
                  <div className="eh-option-group">
                    <label>Content Type / ประเภท</label>
                    <select
                      value={pipelineOptions.contentType}
                      onChange={(e) => setPipelineOptions(p => ({ ...p, contentType: e.target.value as any }))}
                    >
                      <option value="full">Full (เต็มระบบ)</option>
                      <option value="quick">Quick (เร็ว)</option>
                    </select>
                  </div>
                </div>

                <div className="eh-option-group">
                  <label>Word Count / จำนวนคำ</label>
                  <select
                    value={pipelineOptions.wordCount}
                    onChange={(e) => setPipelineOptions(p => ({ ...p, wordCount: parseInt(e.target.value) }))}
                  >
                    <option value="300">300 คำ (สั้น)</option>
                    <option value="500">500 คำ (กลาง)</option>
                    <option value="800">800 คำ (ยาว)</option>
              ...
              {/* Pipeline Info */}
              <div style={{ padding: '10px 14px', borderRadius: '8px', background: 'rgba(0,242,255,0.04)', border: '1px solid rgba(0,242,255,0.1)', fontSize: '0.78rem', color: 'var(--text-muted)' }}>
                🔄 Pipeline: <strong style={{ color: 'var(--primary)' }}>Trend Scan</strong> → <strong style={{ color: '#a78bfa' }}>Lookforward</strong> → <strong style={{ color: '#f59e0b' }}>Image Gen</strong> → <strong style={{ color: '#3b82f6' }}>Facebook Auto Post</strong>
              </div>
            </div>
          </div>
        )}
