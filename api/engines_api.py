...
def run_engine(engine_id):
    ...
    # full-pipeline run now with options
    if engine_id == "full-pipeline" and options.get("mode") == "now":
        if options.get("niche"):
            cmd.extend(["--niche", str(options["niche"])])
        if options.get("tone"):
            cmd.extend(["--tone", str(options["tone"])])
        if options.get("word_count"):
            cmd.extend(["--max-words", str(options["word_count"])])
        if options.get("content_type"):
            cmd.extend(["--mode", str(options["content_type"])])
