...
def _build_cmd(engine_id: str, options: Dict[str, Any]) -> Optional[List[str]]:
...
class CronManager:
    ...
    def _run_loop(self):
        while self.running:
            ...
            # PERF-008: Wake up at the start of the next minute to avoid busy-wait
            now = datetime.now()
            sleep_time = 60 - now.second
            if sleep_time < 1: sleep_time = 60
