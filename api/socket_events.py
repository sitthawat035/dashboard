...
def handle_join_log_stream(data):
    ...
    if not active_log or not os.path.exists(active_log):
        # Emit error event for frontend to handle
        emit("gateway_log_error", {
            "agent_id": agent_id,
            "error": "log_file_not_found",
            "message": "Log file not found. Check gateway is running."
        })
        emit("gateway_log_init", {
            "agent_id": agent_id,
            "lines": [f"\x1b[31m⚠ Log file not found. Check gateway is running.\x1b[0m"],
            "error": "log_file_not_found"
        })
    else:
