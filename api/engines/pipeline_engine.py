...
def get_today_trends(dashboard_dir, niche):
    ...
    if trends_file.exists():
        try:
            with open(trends_file, "r", encoding="utf-8") as f:
                data = json.load(f)
            # If we already scanned today, and it matches the requested niche, reuse it
            if data.get("date") == today_str and "lookforward_topics" in data:
                # If niche is different, we might want to re-scan, but for now we'll just use it
    ...
    print(f"🔍 No trend scan found for today. Running Trend Scan for niche: {niche}...")
    cmd1 = [sys.executable, str(dashboard_dir / "api" / "engines" / "trend_scan" / "daily_trend_scan.py")]
    if niche:
        cmd1.extend(["--niche", niche])
...
def main():
    ...
    parser.add_argument("--niche", default="tech", help="Niche for Trend Scan (e.g. tech, finance)")
    parser.add_argument("--tone", default="professional", help="Tone for Lookforward (e.g. professional, casual)")
    parser.add_argument("--max-words", type=int, default=500, help="Max word count for Lookforward content")
    parser.add_argument("--mode", default="full", help="Lookforward mode: full or quick")
    args = parser.parse_args()

    print("🌟 STARTING FULL AUTOMATED PIPELINE 🌟")
    print(f"Mode: {args.mode} | Sequence: Trend Scan (Niche: {args.niche}) -> Lookforward (Tone: {args.tone}, Max: {args.max_words}w) -> Image Gen -> Facebook Auto Post")
    
    dashboard_dir = Path(__file__).resolve().parent.parent.parent
    
    # 1. Trend Scan & Topic Selection
    trends_data = get_today_trends(dashboard_dir, args.niche)
    if not trends_data:
    ...
    lookforward_mode = "quick" if args.mode == "quick" else "full"
    cmd2 = [
        sys.executable,
        str(dashboard_dir / "api" / "engines" / "lookforward" / "scripts" / "run_pipeline_gemini.py"),
        selected_topic,
        "--tone", args.tone,
        "--mode", lookforward_mode,
    ...
    # 3. Image Gen
    cmd3 = [sys.executable, str(dashboard_dir / "api" / "engines" / "image_gen" / "gen_from_prompt.py"), "--any-date"]
