...
class OpenClawGatewayClient:
    """
    Client for OpenClaw Gateway's built-in AI endpoint.
    Uses the gateway token (from openclaw.json) instead of provider API keys.
    Requires openclaw.json to have:
    ...
    def __init__(
        ...
        self.token = token
        # base_url may already be the full endpoint (e.g. http://host/v1/chat/completions)
        # or just the base host (e.g. http://host). Normalise here.
        if base_url.rstrip("/").endswith("/v1/chat/completions"):
            self._full_url = base_url.rstrip("/")
        else:
            self._full_url = base_url.rstrip("/") + "/v1/chat/completions"
        self.model  = model
    ...
    def chat(
        ...
        try:
            ...
            if "response" in locals() and hasattr(response, "text"):
                self.logger.error(f"OpenClaw Gateway error: {response.text}")
            self.logger.error(f"OpenClaw Gateway request failed: {e}")
            return None
...
def create_ai_client(logger: Optional[logging.Logger] = None):
    ...
    # Option 1: OpenClaw Gateway (PRIMARY - uses gateway token, no provider API key needed)
    oc_token = os.getenv("OPENCLAW_TOKEN")
    oc_url   = os.getenv("OPENCLAW_LOCAL_URL")   # e.g. http://127.0.0.1:2222
    oc_timeout = int(os.getenv("OPENCLAW_TIMEOUT", "300"))
    if oc_token and oc_url:
        oc_model = os.getenv("OPENCLAW_MODEL", "main")
        print_success(f"Using OpenClaw Gateway ({oc_url}) [timeout={oc_timeout}s]")
        return OpenClawGatewayClient(token=oc_token, base_url=oc_url, model=oc_model, timeout=oc_timeout, logger=logger)

    # Option 2: OpenRouter (PRIMARY - All AI including Claude Sonnet via OpenRouter)
