"""
Shopee Scraper v2 - Playwright with proper wait
================================================
รอ JavaScript render เสร็จก่อนดึงข้อมูลสินค้า
ไม่ต้อง login (headless mode)

วิธีใช้:
  python shopee_v2.py --keyword "ของตกแต่งบ้าน" --limit 30
"""
...
def search_products(keyword: str, limit: int = 30, sort: str = "relevant") -> list[dict]:
    ...
    # โหลด cookies (ต้องมี!)
    ck_path = os.path.join(SCRIPT_DIR, "shopee_cookies.json")
    if os.path.exists(ck_path):
        with open(ck_path, "r") as f:
            cookies = json.load(f)
        ctx.add_cookies(cookies)
        print(f"  โหลด cookies ({len(cookies)} cookies)")
    else:
        print("  ⚠️  ไม่พบ cookies - ต้อง login ก่อน!")
        print("  รัน: python shopee_scraper.py --keyword test")
        print("  เพื่อ login และบันทึก cookies")
        browser.close()
        pw.stop()
        return []
    ...
    while len(products) < limit:
        ...
        try:
            # รอ network สงบก่อน (SPA โหลดเสร็จ)
            page.goto(url, wait_until="networkidle", timeout=45000)
        except Exception as e:
            print(f"  ⚠️  Timeout: {e}")
            # ลอง load ใหม่แบบเร็ว
            try:
                page.goto(url, wait_until="domcontentloaded", timeout=30000)
            except Exception as e2:
                print(f"  ❌ โหลดหน้าไม่ได้: {e2}")
                break
...
def extract_products(page) -> list[dict]:
    ...
    # ลอง selector หลายแบบ (อัปเดต 2026)
    selectors = [
        # Shopee 2026 selectors
        "a[data-sqe='link']",
        "[data-sqe='item']",
        "a[data-sqe]",
        "a[href*='-i.']",
    ...
    for sel in selectors:
        try:
            ...
            if items and len(items) >= 3:  # ต้องมีอย่างน้อย 3 ชิ้น
                used_sel = sel
                break
        except Exception:
            continue

    # ถ้ายังไม่เจอ ลองใช้ JavaScript evaluate
    if not items:
        print("    [DEBUG] ลอง JavaScript extraction ...")
        try:
            result = page.evaluate("""() => {
                // หา links ที่มี pattern -i.{shopid}.{itemid}
            ...
            print(f"    [DEBUG] พบ {result} product links")
        except Exception as e:
    ...
    print(f"    [DEBUG] ใช้ selector: {used_sel} ({len(items)} items)")
...
def main():
    parser = argparse.ArgumentParser(
        description="Shopee Scraper v2 - Playwright + proper wait",
    )
