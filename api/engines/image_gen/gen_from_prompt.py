...
READY_TO_POST_BASE = Path(os.getenv(
    "READY_TO_POST_PATH",
    str(dashboard_dir / "data" / "content" / "lookforward" / "ready_to_post")
))
...
def find_latest_package(today_only: bool = True) -> Path | None:
    """
    Auto-detect the most recently modified ready_to_post package.

    Args:
        today_only: If True (default), only look in today's date folder.
                    Fails clearly if today has no package, preventing
                    stale content from being processed in autopilot.
                    Set to False to allow fallback to older packages.
    """
    ...
    if today_only:
        # Strict mode: only look at today's folder
        today_folder = READY_TO_POST_BASE / today_str
        if not today_folder.exists():
            print_error(f"❌ No ready_to_post package for today ({today_str}). Run lookforward first.")
            return None
        date_folders = [today_folder]
    ...
    for date_folder in date_folders:
        ...
        for pkg in packages:
            # Prefer packages that actually have image_prompts
            content_json = pkg / "02_content.json"
            if content_json.exists():
                try:
                    ...
                    pass
            # Also accept packages with prompts_guide.txt
            guide = pkg / "media" / "prompts_guide.txt"
            if guide.exists():
                return pkg
...
def generate_image_hf(prompt: str, model_id: str, hf_token: str) -> bytes | None:
    """Generate an image via HuggingFace Inference API using huggingface_hub."""
    try:
...
def main():
    ...
    parser.add_argument(
        "--any-date",
        action="store_true",
        help="Allow auto-detect to fall back to packages from previous days (not just today)",
    )
    ...
    if args.package_path:
        ...
        today_only = not args.any_date
        mode_label = "any date" if args.any_date else "today only"
        print_info(f"🔍 Auto-detecting latest ready_to_post package ({mode_label})...")
        package = find_latest_package(today_only=today_only)
        if not package:
            print_error(f"❌ No ready_to_post package found in:\n   {READY_TO_POST_BASE}")
            sys.exit(1)
