...
POST_HISTORY_FILE = root_dir / "data" / "post_history.json"


def save_post_log(status: str, topic: str = "", message_preview: str = "",
                  post_id: str = "", error: str = "", image_count: int = 0):
    """Append one entry to persistent post_history.json."""
    ...
    history.insert(0, {
        ...
        "topic": topic,
        "message_preview": message_preview[:120] if message_preview else "",
        "post_id": post_id,
        "error": error,
...
def post_to_facebook_graph(page_id: str, access_token: str, message: str, image_paths: list = None):
    ...
    try:
        if not image_paths or len(image_paths) == 0:
            ...
            result = {"status": "success", "id": res_data.get('id')}
            save_post_log("success", message_preview=message,
                          post_id=result["id"] or "",
            ...
            result = {"status": "success", "id": res_data.get('post_id', res_data.get('id'))}
            save_post_log("success", message_preview=message,
                          post_id=result["id"] or "",
                          image_count=len(image_paths) if image_paths else 0)
            return result
        ...
        save_post_log("error", message_preview=message, error=str(e),
                      image_count=len(image_paths) if image_paths else 0)
        return err_result
...
async def post_from_pending():
    ...
    dashboard_dir = Path(__file__).resolve().parent.parent.parent.parent
    ready_dir = dashboard_dir / "data" / "content" / "lookforward" / "ready_to_post"
    ...
    # Extract topic from package folder name (best-effort)
    topic = latest_package.name.replace("_", " ").rsplit("_", 1)[0]  # strip timestamp

    # Update log with topic
    save_post_log(
