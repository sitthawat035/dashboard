...
def main():
    ...
    parser.add_argument(
        "--run-pipeline",
        action="store_true",
        help="After scan, auto-trigger run_pipeline_gemini.py with top topic → creates ready_to_post package",
    )
    args = parser.parse_args()
    result = scan_daily_trends(niche=args.niche, custom_niche=args.custom_niche)

    if args.run_pipeline and result:
        topics = result.get("lookforward_topics", [])
        if not topics:
            print("\n⚠️ No topics found — skipping pipeline trigger.")
            return
        top_topic = topics[0].get("topic", "")
        if not top_topic:
            print("\n⚠️ Empty topic — skipping pipeline trigger.")
            return

        pipeline_script = Path(__file__).resolve().parent.parent / "lookforward" / "scripts" / "run_pipeline_gemini.py"
        venv_python = Path(__file__).resolve().parent.parent.parent.parent / ".venv" / "bin" / "python3"
        python_bin = str(venv_python) if venv_python.exists() else sys.executable

        print(f"\n🚀 Auto-triggering lookforward pipeline...")
        print(f"   Topic: {top_topic}")
        print(f"   Script: {pipeline_script}")
        print()

        subprocess.run(
            [python_bin, str(pipeline_script), "--topic", top_topic],
            check=False,
        )
